...
##### LAB 7 PATHS (OG) #####
coPath = r"\\DESKTOP-FRV6AA4\Users\Somby Lab\Desktop\Tiam\Files\P1Response.txt"
selfPath = r"\\DESKTOP-FRV6AA4\Users\Somby Lab\Desktop\Tiam\Files\P2Response.txt"
...
r_coPath = r"\\DESKTOP-FRV6AA4\Users\Somby Lab\Desktop\Tiam\Files\ready.txt"
r_selfPath = r"\\DESKTOP-FRV6AA4\Users\Somby Lab\Desktop\Tiam\Files\ready2.txt"
...
part_file = r"\\DESKTOP-FRV6AA4\Users\Somby Lab\Desktop\Tiam\Files\part_file2.txt"
WhatsAppPath = ["cmd", "/C", "start whatsapp://send?phone=+4368120770558"]
...
def start(phase_start):
    ...
    welcome(phase)
    with open(r_selfPath, 'w') as ready2:
        ready2.write('ok')
    with open(r4_path, 'w') as ready2Controll:
        ready2Controll.write('ok')
    pressedSpace.draw()
    waiting_signal.draw()
    win.flip()
...
def readofferJoint():
    while True:
        if os.path.getsize(offer_path) > 0:
            with open(offer_path, 'r') as offerJoint:
                offer = offerJoint.read()
            return offer
            break
        else:
            sleep(0.5)

def readoffer(phase): #Phase only for P2
    ### Phase here only for P2 ###
    if phase == 0:
        offerPath = r4_path
    elif phase == 1:
        offerPath = offer_path
    else:
        offerPath = r4_path
### Phase here only for P2 ###
    waiting_signal = visual.TextStim(win, text='.', height = 50, pos=[0,-150])
    ...
    while True:
        ...
        if timerOffer.getTime() <= 0:
            if phase == 0 or phase == 2:
                ...
                with open(offerPath, 'w') as offerP2:
                    offerP2.write(str(offer))
                extortionist1.accumulated.append(offer)
            else:
                offer = readofferJoint()
                extortionist1.accumulated.append(offer)
            #extortionist1.accumulated.append(offer)
            #try:
            ...
            if len(Offers) > 0:
                ...
                core.wait(1)
                if phase == 1:
                    with open(offerPath,"w") as offerJoint:
                        pass
                break
            #except:
            #    sleep(0.2)
...
def write(reply):
    with open(selfPath, 'w') as p2res:
        p2res.write(str(reply))
        p2res.close()

#Send final team decision to Decision.txt
def sendDecision(path): ### Phase here only for P2 ###
# ### Phase here only for P2 ###
#     if phase == 0:
#         dec_path = dec_path2
#     elif phase == 1:
...
# ### Phase here only for P2 ###
    if len(Readers) > 0:
        ...
        Decision = Be[0]
        P2Decision = Decision[0]
        with open(path, 'w') as decision:
            decision.write(str(P2Decision))
            decision.close()
        extortionist1.responses.append(P2Decision)

            #with open(dec_path, 'w') as decision:
            #    decision.write(str(Dec))
            #    decision.close()
        #with open(offer_path, 'w'):
        #    pass
    ...
    if Offers[0] != 0:
        while timer.getTime() > 0:
            ...
            if timer.getTime() <= 0:
                ...
                sendDecision(dec_path2) #Phase here only for P2
                win.flip()
                core.wait(2.0)
                break
            if len(keys) > 0:
...
def read():
    ...
    index = 0
    decisionIndex = 0 #Only P2 for checking if P1 was too late or there was disagreement
    timer = core.CountdownTimer(decisionTimer)
    if Offers[0] != 0:
        while True:
            if index > 0:
                #with open(coPath, "w") as p1res: #empty coPath at index == 1 for JD
                #    pass
                makedicts('Accumulated', Accumulated[-1])
                break
            while timer.getTime() > 0:
                ...
                if len(keys) > 0:
                    ...
                    while True:
                        ...
                        if os.path.getsize(coPath) > 0:
                            with open(coPath, 'r') as p1res:
                                P1 = p1res.read()
                            if P1 and not Readers:
                                if pressedKey == P1:
                                    index += 1 #index += 1 only for JD
                                    Readers.append([pressedKey, P1])
                                    makedicts('TimeStamp', 0)
                                    if pressedKey == 'a':
                                        you_accepted.draw()
                                        win.flip()
                                ...
                                if pressedKey != P1:
                                    timeLocal=time_ms(timeDecision)
                                    if P1 == 'a_rand':
                                        index += 1 #index += 1 here for P2
                                        if decisionIndex == 0: #Only P2
                                            partner_failed.draw()
                                            win.flip()
                                            core.wait(3.0)
                                        accepted.draw()
                                    ...
                                    if P1 == 'r_rand':
                                        ...
                                        break
                                    if P1 == 'r':
                                        #+++++++++++++++++++++++++++++weg damit ohne lappi#
                                        with open(coPath, 'w') as p1res:
                                            pass
                                        #+++++++++++++++++++++++++++++weg damit ohne lappi#
                                        decisionIndex += 1 #Only P2
                                        partner_rejected.draw()
                                    if P1 == 'a':
                                        #+++++++++++++++++++++++++++++weg damit ohne lappi#
                                        with open(coPath, 'w') as p1res:
...
with pd.ExcelWriter('JD_P2.xlsx', engine='openpyxl',mode='a') as writer: #Excel name JD_P2 for JD_P2
...
if experiment_done == 1:
    ...
    import HEXACO_P2
    def hexaco():
        HEXACO_P2.participant_number()
        HEXACO_P2.get_empty_col()
        HEXACO_P2.welcome()
        HEXACO_P2.save_workbook()
        HEXACO_P2.bye()
        HEXACO_P2.close_workbook()
        HEXACO_P2.thank_you()
    hexaco()
win.close()
core.quit()